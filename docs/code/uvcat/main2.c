...
void open_cb(uv_fs_t* fs_open_req) {
    ...
    if (fs_open_req->result < 0) { // error // fs_open_req->file?
        ...
        printf("fs_read_wrapper address: %p\n", fs_read_wrapper);
        printf("fs_read_wrapper->req address: %p\n", &fs_read_wrapper->req);
        printf("fs_read_wrapper->buf address: %p\n", &fs_read_wrapper->buf);
    }
...
void read_cb(uv_fs_t* fs_read_req) {
    printf("read_cb enter \n");

    fs_t *rt = (fs_t*)fs_read_req;
    printf("fs_read_req address: %p\n", fs_read_req);
    printf("rt->req address: %p\n", &rt->req);
    printf("rt->buf address: %p\n", &rt->buf);
